...
public class FlintonSewersAlchemist extends StaticHuman {
	...
	private static final int SAMPLING_SLUDGE_XP_REWARD = 350;

	private final InteractionSequence startSamplingSludgeInteraction;
	private final InteractionSequence completeSamplingSludgeInteraction;
	...
	public FlintonSewersAlchemist(Location location) {
		...
		startSamplingSludgeInteraction = new InteractionSequence(7) {
			@Override
		...
		completeSamplingSludgeInteraction = new InteractionSequence(2) {
			@Override
			protected void onAdvance(PlayerCharacter pc, int interactionIndex) {
				switch (interactionIndex) {
				case 0:
