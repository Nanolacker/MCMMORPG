package com.mcmmorpg.impl.npcs;

import org.bukkit.ChatColor;
import org.bukkit.Location;
import org.bukkit.Sound;

import com.mcmmorpg.common.character.AbstractCharacter;
import com.mcmmorpg.common.character.PlayerCharacter;
import com.mcmmorpg.common.quest.QuestMarker;
import com.mcmmorpg.common.quest.QuestObjective;
import com.mcmmorpg.common.quest.QuestStatus;
import com.mcmmorpg.common.sound.Noise;
import com.mcmmorpg.common.ui.InteractionSequence;
import com.mcmmorpg.impl.Quests;

public class GuardThomas extends StaticHuman {

	private static final int LEVEL = 25;
	private static final String TEXTURE_DATA = "";
	private static final String TEXTURE_SIGNATURE = "";
	private static final Noise SPEAK_NOISE = new Noise(Sound.ENTITY_VILLAGER_AMBIENT, 1, 0.75f);

	private final InteractionSequence completeIntoTheSewersInteraction;
	private final InteractionSequence startClearingTheSewersInteraction;
	private final InteractionSequence completeClearingTheSewersInteraction;

	public GuardThomas(Location location) {
		super(ChatColor.GREEN + "Guard James", LEVEL, location, TEXTURE_DATA, TEXTURE_SIGNATURE);
		QuestMarker.createMarker(location.clone().add(0, 2.25, 0));
		completeIntoTheSewersInteraction = new InteractionSequence(2) {
			@Override
			protected void onAdvance(PlayerCharacter pc, int messageIndex) {
				switch (messageIndex) {
				case 0:
					break;
				}
			}
		};
		startClearingTheSewersInteraction = new InteractionSequence(2) {
			@Override
			protected void onAdvance(PlayerCharacter pc, int messageIndex) {
				switch (messageIndex) {
				case 0:
					say("Insert dialogue", pc);
					break;
				case 1:
					Quests.CLEARING_THE_SEWERS.start(pc);
					break;
				}
			}
		};
		completeClearingTheSewersInteraction = new InteractionSequence(2) {
			@Override
			protected void onAdvance(PlayerCharacter pc, int messageIndex) {
				switch (messageIndex) {
				case 0:
					say("Insert dialogue", pc);
					break;
				case 1:
					Quests.CLEARING_THE_SEWERS.getObjective(4).complete(pc);
					break;
				}
			}
		};
	}

	@Override
	protected void onInteract(PlayerCharacter pc) {
		if (Quests.INTO_THE_SEWERS.compareStatus(pc, QuestStatus.IN_PROGRESS)) {
			completeIntoTheSewersInteraction.advance(pc);
		} else if (Quests.INTO_THE_SEWERS.compareStatus(pc, QuestStatus.COMPLETED)) {
			if (Quests.CLEARING_THE_SEWERS.compareStatus(pc, QuestStatus.NOT_STARTED)) {
				startClearingTheSewersInteraction.advance(pc);
			} else if (Quests.CLEARING_THE_SEWERS.compareStatus(pc, QuestStatus.COMPLETED)) {
				say("How do you do?", pc);
			} else {
				QuestObjective[] objectives = Quests.CLEARING_THE_SEWERS.getObjectives();
				boolean readyToTurnIn = true;
				for (int i = 0; i < objectives.length - 1; i++) {
					if (!objectives[i].isComplete(pc)) {
						readyToTurnIn = false;
						break;
					}
				}
				if (readyToTurnIn) {
					completeClearingTheSewersInteraction.advance(pc);
				} else {
					say("Go clear the sewers.", pc);
				}
			}
		}

	}

	@Override
	public void say(String dialogue, AbstractCharacter recipient) {
		super.say(dialogue, recipient);
		SPEAK_NOISE.play((PlayerCharacter) recipient);
	}

}
