...
public class MelcherBartender extends StaticHuman {
	...
	private static final int PEST_CONTROL_XP = 75;
	...
	public MelcherBartender(Location location) {
		...
		startCalmingTheTavernInteraction = new InteractionSequence(5) {
			@Override
		...
		completeCalmingTheTavernInteraction = new InteractionSequence(6) {
			@Override
			protected void onAdvance(PlayerCharacter pc, int interactionIndex) {
				switch (interactionIndex) {
				case 0:
		...
		completePestControlInteraction = new InteractionSequence(4) {
			...
			protected void onAdvance(PlayerCharacter pc, int interactionIndex) {
				...
				case 3:
					...
					pc.giveItem(Items.MELCHER_MEAD);
	...
	protected void onInteract(PlayerCharacter pc) {
		if (Quests.CALMING_THE_TAVERN.compareStatus(pc, QuestStatus.NOT_STARTED)) {
			startCalmingTheTavernInteraction.advance(pc);
		} else if (Quests.CALMING_THE_TAVERN.compareStatus(pc, QuestStatus.IN_PROGRESS)
				&& Quests.CALMING_THE_TAVERN.getObjective(0).isComplete(pc)) {
			completeCalmingTheTavernInteraction.advance(pc);
